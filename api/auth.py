...
class JWTAuth(HTTPAuth):
    ...
    async def __call__(self, request: Request) -> dict[Any, Any] | None:
        if (
            data := decode_jwt(get_token(request), audience=self.audience)
        ) is None and self.force_valid:
...
def _get_user_dependency(
    *args: Column[Any],
) -> Callable[[str, Session | None], Awaitable[User]]:
    async def default_dependency(
        user_id: str, session: Session | None = public_auth
    ) -> User:
...
def _get_user_privileged_dependency(
    *args: Column[Any],
) -> Callable[[str, Session], Awaitable[User]]:
    async def self_or_admin_dependency(
        user_id: str, session: Session = user_auth
    ) -> User:
...
def get_user(*args: Column[Any], require_self_or_admin: bool = False) -> Any:
    return Depends(
        _get_user_privileged_dependency(*args)
        if require_self_or_admin
        else _get_user_dependency(*args)
    )
