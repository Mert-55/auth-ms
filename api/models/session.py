...
class Session(Base):
    ...
    async def invalidate_access_token(self) -> None:
        await redis.setex(
            f"session_logout:{self.refresh_token}", settings.access_token_ttl, 1
        )
    ...
    async def refresh(refresh_token: str) -> tuple[Session, str, str]:
        ...
        session: Session | None = await db.get(
            Session, Session.user, refresh_token=token_hash
        )
        if not session:
            raise ValueError("Invalid refresh token")
        if utcnow() > session.last_update + timedelta(
...
async def clean_expired_sessions() -> None:
    await db.exec(
        delete(Session).where(
            Session.last_update
            < utcnow() - timedelta(seconds=settings.refresh_token_ttl)
        )
    )
