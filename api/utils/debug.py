...
def _check_response_schema(
    method: str, route: APIRoute, status_code: int, body: bytes
) -> None:
    ...
    if status_code not in route.responses:
        print(
            f"[{method} {route.path}] no response schema defined for status code {status_code}"
        )
        return
    ...
    if "model" in response:
        ...
        try:
            ...
            print(
                f"[{method} {route.path}] response schema validation failed ({status_code}):\n{e}"
            )
    elif json.loads(body) not in (
        v.get("value")
        for v in response.get("content", {})
