...
class Message:
    ...
    async def send(
        self, recipient: str, *, reply_to: str | None = None, **kwargs: Any
    ) -> None:
...
VERIFY_EMAIL = Message(
    title="Willkommen bei der Bootstrap Academy!", template="verify_email.html"
)
RESET_PASSWORD = Message(
    title="Passwort zurücksetzen - Bootstrap Academy", template="reset_password.html"
)
...
async def send_email(
    recipient: str, title: str, body: str, *, reply_to: str | None = None
) -> None:
    if not await check_email_deliverability(recipient):
        raise ValueError("Invalid email address")

    print(f"Sending email to {recipient} ({title})")
