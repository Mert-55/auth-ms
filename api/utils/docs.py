...
def responses(
    default: type, *args: Type[APIException]
    ...
    for code, excs in exceptions.items():
        ...
        for i, exc in enumerate(excs):
            name = (
                exc.__name__
                if len(excs) == 1
                else f"{exc.__name__} ({i + 1}/{len(excs)})"
            )
            examples[name] = {
...
def get_example(arg: type[BaseModel]) -> dict:
    if hasattr(arg.model_config, "json_schema_extra"):
        return arg.model_config.json_schema_extra.get("example", {})
    return {}


def example(*args: Type[BaseModel], **kwargs: Any) -> dict:
    ex = dict(e for arg in args for e in get_example(arg).items())
    return {"json_schema_extra": {"example": ex | kwargs}}
