...
class DB:
    def __init__(self, url: str, **kwargs: Any):
        ...
        self._close_event: ContextVar[Event | None] = ContextVar(
            "close_event", default=None
        )
    ...
    async def stream(self, statement: Executable | str) -> AsyncIterator[Any]:
        ...
        return cast(
            AsyncIterator[Any], (await self.session.stream(statement)).scalars()
        )
    ...
    async def exists(
        self, statement: Executable | str, *args: Column[Any], **kwargs: Any
    ) -> bool:
        ...
        return cast(
            bool,
            await self.first(
                exists(cast(Executable, statement), *args, **kwargs).select()
            ),
        )
    ...
    async def count(self, statement: Select) -> int:
        ...
        return cast(
            int, await self.first(select(count()).select_from(statement.subquery()))
        )
